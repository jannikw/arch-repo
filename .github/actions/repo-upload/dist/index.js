...
const artifactsDir = path_1.join(workspace, core.getInput("artifacts", { required: true }));
const repositoryDir = path_1.join(workspace, core.getInput("repository", { required: true }));
action().catch((error) => {
    core.setFailed(error.message);
});
async function action() {
    if (!(await utils_1.checkFileExists(artifactsDir))) {
        core.info("no artifacts to upload");
        return;
    }
    const paths = [];
    for (const artifact of await utils_1.getDirSubdirs(artifactsDir)) {
        core.info(`checking in ${artifact}`);
        for (const artifactFile of await utils_1.getDirFiles(path_1.join(artifactsDir, artifact))) {
            const path = path_1.join(artifactsDir, artifact, artifactFile);
